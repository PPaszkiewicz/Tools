...
/** Reflects from SELF - [IFieldReflector] delegate with null argument. */
class PrivateContainerReflector : PrivateContainer(), IFieldReflector by FieldReflector<PrivateContainer>(){
    /** Reflects str from self.*/
    val str : String by this
    ...
    val overload by invoke<String>(String::class.java)  // can also use invoke methods name
    ...
    val listOverloadInt by this<List<*>>()
}

/** Reflects fields from class that cannot be extended - [IFieldReflector] delegate with argument. */
class PrivateObjectReflector(val privateObject: PrivateObject) : IFieldReflector by FieldReflector(privateObject){
    var double : Double by this
    val privateMethod by invoke()
}

/** Reflects fields from class that cannot be extended without implementing [IFieldReflector]. */
